...
testExitCodes()
{
  ...
  # Check that running with a short clear flag and no key results in an error
  # exit code
  xeno -c > /dev/null 2>&1
  result=$?
  assertEquals "xeno config with -c should exit with code 1" 1 ${result}

  # Check that running with a long clear flag and not key results in an error
  # exit code
